...
3. **Adaptações de Código:**
   - usado `<HashRouter>` no lugar de `<BrowserRouter>` para evitar erros 404 nas rotas (como ocorreu em teste).
   - usado `localStorage` no lugar de arquivos locais (`json-server`), pois o Github Pages suporta apenas páginas estáticas.
